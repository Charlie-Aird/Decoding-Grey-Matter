function [basic_stats, advanced_stats]=analyse_neurite_morphology_tree(trees, outputfolder)
...
for i = 1:Ncells
    ...
    tmp.branch_CV = zeros(tmp.number_of_branches,1);
    
    ipar = ipar_tree (tree);
    
    for ii=1:tmp.number_of_branches
        
        branch_node_idx = flip( ipar(bsect (ii,2), ipar(bsect (ii,2),:)>=bsect (ii,1)) );
        ...
        tmp.branch_CV(ii,:) = nanstd(tree.D(branch_node_idx))/ nanmean(tree.D(branch_node_idx));
        ...
        try
            ...
            tmp.branch_cumulative_curve_length{ii} = sum(sL(branch_node_idx));
            tmp.branch_curvature_radius{ii} = R;
            tmp.branch_curvature_vec{ii} = K;
        catch
            tmp.branch_cumulative_curve_length{ii} = [];
            tmp.branch_curvature_radius{ii} = [];
            tmp.branch_curvature_vec{ii} = [];    
        end
